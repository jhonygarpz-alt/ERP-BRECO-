import { unidadToRow } from './mappers';
...
const COLUMNAS: { clave: string; campo: string }[] = [
  ...
  { clave: 'tipo de unidad (clave sat)', campo: 'tipo' },
  ...
  { clave: 'transmision', campo: 'tipoTransmision' },
  { clave: 'motor', campo: 'tipoMotor' },
];

const ENCABEZADOS = [
  'Codigo', 'Placas', 'Tipo de Unidad (clave SAT)', 'Marca', 'Modelo', 'Anio', 'Activa', 'Rentada',
  'Es Permisionario', 'Descripcion', 'Sucursal', 'Identidad Satelital', 'Identificador de Convoy',
  'Numero de Serie', 'Color', 'Grupo de Unidades', 'Largo (m)', 'Ancho (m)', 'Alto (m)', 'Capacidad (Kg)',
  'Numero de Ejes', 'Peso Tara (Ton)', 'Transmision', 'Motor',
];

export function descargarPlantillaUnidades(): void {
  descargarPlantilla({
    nombreArchivo: 'Plantilla_Importar_Unidades.xlsx',
    nombreHojaDatos: 'Unidades',
    encabezados: ENCABEZADOS,
    tituloInstrucciones: 'Instrucciones para llenar la plantilla de Unidades',
    notasGenerales: [
      '1. No cambies el nombre ni el orden de las columnas de la hoja "Unidades".',
      '2. Llena una fila por cada unidad, empezando en la fila 2.',
      '3. Guarda el archivo en formato .xlsx antes de subirlo.',
      '4. El Codigo (economico) no se puede repetir.',
      '5. Foto, documentos con vencimiento, archivos adicionales y seguros se agregan despues, editando cada unidad ya importada.',
    ],
    instrucciones: [
      ['Codigo', 'Obligatorio. El numero economico de la unidad; no se puede repetir.'],
      ['Placas', 'Opcional.'],
      ['Tipo de Unidad (clave SAT)', 'Opcional. Usa la clave del catalogo SAT "c_ConfigAutotransporte" (visible al editar una unidad en el sistema); ej. "C2", "T3S2".'],
      ['Marca', 'Opcional.'],
      ['Modelo', 'Opcional.'],
...
export async function leerUnidadesExcel(file: File): Promise<{ totalFilasHoja: number; filas: FilaImport<Unidad>[] }> {
  const { encabezados, filas2d } = await leerFilasHoja(file, 'Unidades');
  ...
  for (let i = 1; i < filas2d.length; i++) {
    ...
    const item: Unidad = {
      ...
      tipoTransmision: texto(col('tipoTransmision', fila)),
      tipoMotor: texto(col('tipoMotor', fila)),
      tipoCombustible: '',
      tarjetaCombustible1: '',
      tarjetaCombustible2: '',
      tarjetaCombustible3: '',
...
export function marcarDuplicadosUnidades(filas: FilaImport<Unidad>[], existentes: Unidad[]): FilaImport<Unidad>[] {
  const vistos = new Set<string>();
  return filas.map((f) => {
    const errores = [...f.errores];
    const economico = f.item.economico.trim();
    if (economico) {
      if (existentes.some((u) => u.economico.trim() === economico)) errores.push(`El codigo "${economico}" ya existe.`);
...
export async function guardarUnidadesImportadas(items: Unidad[], onProgreso?: (hecho: number, total: number) => void): Promise<void> {
  await guardarEnLotes('unidades', items, unidadToRow, onProgreso);
}

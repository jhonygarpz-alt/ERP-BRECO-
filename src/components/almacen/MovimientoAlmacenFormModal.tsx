...
import type { LineaArticuloAlmacen, MovimientoAlmacen } from '../../types';
...
export function MovimientoAlmacenFormModal({
  editing,
  soloLectura,
  onClose,
  onGuardar,
}: {
  editing: MovimientoAlmacen | null;
  soloLectura: boolean;
  onClose: () => void;
  onGuardar: (datos: MovimientoAlmacen) => void;
}) {
  const { movimientosAlmacen, tiposMovimientoAlmacen, almacenes, proveedores } = useData();

  const [fecha, setFecha] = useState(editing?.fecha ?? hoyISO());
  const [tipoMovimientoId, setTipoMovimientoId] = useState(editing?.tipoMovimientoId ?? '');
  const [almacenId, setAlmacenId] = useState(editing?.almacenId ?? '');
  const [almacenDestinoId, setAlmacenDestinoId] = useState(editing?.almacenDestinoId ?? '');
  ...
  const [almacenDestinoPickerOpen, setAlmacenDestinoPickerOpen] = useState(false);
  ...
  const almacenDestinoSeleccionado = almacenes.items.find((a) => a.id === almacenDestinoId);
  ...
  function abrirEditarLinea(l: LineaArticuloAlmacen) {
    setLineaEditando(l);
    setLineaModalOpen(true);
  }
  ...
  function eliminarLinea(id: string) {
    setLineas((actual) => actual.filter((l) => l.id !== id));
  }
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    if (!almacenId) {
      setError('Selecciona el Almacen.');
      return;
    }
    if (lineas.length === 0) {
    ...
    setError('');
    onGuardar({
      id: editing?.id ?? uid('mov'),
      folio: editing?.folio ?? nextFolioAlmacen(movimientosAlmacen.items, 'MOV-'),
      fecha,
      tipoMovimientoId,
      almacenId,
      almacenDestinoId: almacenDestinoId || undefined,
  ...
  return (
    <Modal
      title={soloLectura ? 'Consultar movimiento de almacen' : editing ? 'Editando movimiento de almacen' : 'Agregando movimiento de almacen'}
      onClose={onClose}
      wide="xl"
    >
        ...
        <fieldset disabled={soloLectura} className="space-y-4">
          <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
            ...
            <Field label="Tipo Movimiento">
              <Select value={tipoMovimientoId} onChange={(e) => setTipoMovimientoId(e.target.value)}>
                <option value="">Selecciona...</option>
                {tiposMovimientoAlmacen.items
                  .filter((t) => t.activo)
                  .map((t) => (
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="Almacen Destino (solo traspasos)">
              <div className="flex items-center gap-2">
                <Input
                  readOnly
                  value={almacenDestinoSeleccionado ? `${almacenDestinoSeleccionado.codigo} - ${almacenDestinoSeleccionado.nombre}` : ''}
                  placeholder="Sin destino (no es traspaso)"
                />
                <ToolbarButton type="button" onClick={() => setAlmacenDestinoPickerOpen(true)}>
                  ...
                </ToolbarButton>
                {almacenDestinoId && (
                  <GhostButton type="button" onClick={() => setAlmacenDestinoId('')}>
                    Quitar
                  </GhostButton>
                )}
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <Field label="Proveedor (opcional)">
              <div className="flex items-center gap-2">
                ...
                {proveedorId && (
                  <GhostButton type="button" onClick={() => setProveedorId('')}>
                    Quitar
                  </GhostButton>
                )}
              </div>
          ...
          <div>
            <div className="mb-2 flex items-center justify-between">
              ...
              <ToolbarButton type="button" onClick={abrirNuevaLinea}>
                  ...
                  {lineas.map((l) => (
                    <tr key={l.id} className="border-t border-line-800/70 text-ink-300">
                      ...
                      <td className="px-3 py-1.5">
                        <div className="flex items-center gap-2">
                          <button type="button" onClick={() => abrirEditarLinea(l)} className="text-ink-600 hover:text-breco-500">
                            <Pencil size={13} />
                          </button>
      ...
      {almacenDestinoPickerOpen && (
        <ListaSeleccionModal
          title="Buscar almacen destino"
          items={almacenes.items.filter((a) => a.activo && a.id !== almacenId)}
          filtro={(a, t) => !t || a.nombre.toLowerCase().includes(t) || a.codigo.toLowerCase().includes(t)}
          renderRow={(a) => (
            <>
              <td className="px-3 py-2 font-mono text-xs text-breco-400">{a.codigo}</td>

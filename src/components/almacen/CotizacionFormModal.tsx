...
import type { Cotizacion, LineaArticuloAlmacen } from '../../types';
...
export function CotizacionFormModal({
  ...
  soloLectura: boolean;
  onClose: () => void;
  onGuardar: (datos: Cotizacion) => void;
}) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    onGuardar({
      id: editing?.id ?? uid('cot'),
      folio: editing?.folio ?? nextFolioAlmacen(cotizaciones.items, 'COT-'),
      fecha,
  ...
  return (
    <Modal title={soloLectura ? 'Consultar cotizacion' : editing ? 'Editando cotizacion' : 'Agregando cotizacion'} onClose={onClose} wide="xl">
            ...
            <div className="overflow-auto rounded-xl border border-line-800">
              <table className="w-full text-left text-sm">
                ...
                <tbody>
                  {lineas.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-3 py-6 text-center text-ink-600">

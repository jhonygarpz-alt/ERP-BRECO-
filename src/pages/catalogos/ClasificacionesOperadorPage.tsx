...
import type { ClasificacionOperador } from '../../types';
...
const emptyForm: Omit<ClasificacionOperador, 'id'> = {
  codigo: '',
  clasificacion: '',
  activo: true,
};

export function ClasificacionesOperadorPage() {
  const { clasificacionesOperador } = useData();
  ...
  const [editing, setEditing] = useState<ClasificacionOperador | null>(null);
  const [form, setForm] = useState(emptyForm);
  const [error, setError] = useState('');

  const filtered = useMemo(
    () =>
      clasificacionesOperador.items.filter(
        (c) =>
          c.clasificacion.toLowerCase().includes(search.toLowerCase()) ||
          c.codigo.toLowerCase().includes(search.toLowerCase()),
      ),
    [clasificacionesOperador.items, search],
  );

  function siguienteCodigo(): string {
    const numeros = clasificacionesOperador.items
  ...
  function buscarDuplicado(): string | null {
    ...
    const otros = clasificacionesOperador.items.filter((c) => c.id !== editing?.id);
  ...
  function openEdit(c: ClasificacionOperador) {
    setEditing(c);
    setForm(c);
    setError('');
    setModalOpen(true);
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    if (editing) {
      clasificacionesOperador.update(editing.id, form);
    } else {
      clasificacionesOperador.add({ id: uid('clo'), ...form });
    }
    setModalOpen(false);
  }

  function handleDelete(c: ClasificacionOperador) {
    if (confirm(`Eliminar la clasificacion "${c.clasificacion}"?`)) clasificacionesOperador.remove(c.id);
  }

  const columns: Column<ClasificacionOperador>[] = [
  ...
  return (
    <div>
      <PageHeader
        title="Clasificaciones de Operador"
        subtitle="Clasificaciones disponibles para los operadores (ej. Propio, Permisionario, Torton, Full, Tractocamion, Camionetas)."
        search={search}
      ...
      {modalOpen && (
        <Modal
          title={editing ? 'Editando Clasificacion de Operador' : 'Agregando Clasificacion de Operador'}
